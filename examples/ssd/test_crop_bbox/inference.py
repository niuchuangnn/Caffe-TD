...
if __name__ == "__main__":
    caffe.set_device(1)
    caffe.set_mode_gpu()
    # caffe.set_random_seed(12345)
    net_def = 'test_crop_bbox.prototxt'
    net = caffe.Net(net_def, caffe.TRAIN)

    net.forward()

    conv = net.blobs['conv5_3'].data
    conv_crop = net.blobs['conv_crop'].data
    conv_crop_test = np.zeros(conv.shape)
    width_l = conv.shape[2]
    height_l = conv.shape[3]
    ...
    for i in xrange(num_imgs):
        ...
        for j in xrange(num_selected_bbox):
            ...
            if gt_bbox[0] == i:
                ...
                # test crop_bbox layer
                xmin_l = int(np.floor(gt_bbox[3] * width_l))
                ymin_l = int(np.floor(gt_bbox[4] * height_l))
                xmax_l = int(np.ceil(gt_bbox[5] * width_l))
                ymax_l = int(np.ceil(gt_bbox[6] * height_l))

                conv_crop_test[i, :, ymin_l:ymax_l, xmin_l:xmax_l] = conv[i, :, ymin_l:ymax_l, xmin_l:xmax_l]

        print conv_crop.max()
        print np.abs(conv_crop[i] - conv_crop_test[i]).sum()

        plt.figure(2)
        plt.imshow(binary_seg_color)
        plt.title(str(binary_seg_labels))

        plt.show()

    print np.abs(conv_crop - conv_crop_test).sum() == 0
