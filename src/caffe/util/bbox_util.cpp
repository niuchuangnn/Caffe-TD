...
void GetGroundTruth(const Dtype* gt_data, const int num_gt,
  ...
  for (int i = 0; i < num_gt; ++i) {
    ...
    void GetAllClassLabels(const map<int, vector<NormalizedBBox> > all_gt_bboxes, map<int, vector<int>>* cls_labels){
        int label;
        cls_labels->clear();
        vector<int>::iterator iter_label;
        map<int, std::vector<caffe::NormalizedBBox>>::const_iterator iter_gt;
        vector<int> label_ini;
        for (iter_gt = all_gt_bboxes.begin(); iter_gt != all_gt_bboxes.end(); iter_gt++){
          int img_id = iter_gt->first;
          vector<NormalizedBBox> im_bboxes = iter_gt->second;
          for (int j = 0; j < im_bboxes.size(); ++j) {
            label = static_cast<int>(im_bboxes[j].label());
              // If the im_id does not exist in cls_labels, and push it into the cls_labels as a initial label.
              if ((*cls_labels).find(img_id) == (*cls_labels).end()) {
                  label_ini.clear();
                  label_ini.push_back(label);
                  (*cls_labels)[img_id] = label_ini;
              } else {
                  // If the label does not exist and push it into the cls_labels vector.
                  iter_label = std::find((*cls_labels)[img_id].begin(), (*cls_labels)[img_id].end(), label);
                  if (iter_label == (*cls_labels)[img_id].end()) {
                      (*cls_labels)[img_id].push_back(label);
                  }
              }
          }
        }
    ...
    void RandomSelectLabel(const map<int, vector<int> > cls_labels, map<int, int>* selected_labels){
        selected_labels->clear();
        map<int, std::vector<int>>::const_iterator iter;
        for (iter = cls_labels.begin(); iter != cls_labels.end(); ++iter){
            int img_id = iter->first;
            vector<int> labels = iter->second;
//            std::cout << labels.size() << std::endl;
            srand((unsigned)time(0));
            int select_bbox_id = rand() % labels.size();
            (*selected_labels)[img_id] = labels[select_bbox_id];
        }
    }

    int NumClsBBox(const map<int, vector<NormalizedBBox> > all_gt_bboxes, const map<int, int> selected_labels){
        int num_cls_specific_box = 0;
        map<int, std::vector<caffe::NormalizedBBox>>::const_iterator iter;
        for (iter = all_gt_bboxes.begin(); iter != all_gt_bboxes.end(); ++iter){
            int img_id = iter->first;
            vector<NormalizedBBox> bboxes_im = iter->second;
            for (int i = 0; i < bboxes_im.size(); ++i){
                int label = bboxes_im[i].label();
                if (label == selected_labels.at(img_id)) {
                    num_cls_specific_box++;
                }
            }
        }
        return num_cls_specific_box;
    }

    template void GenerateClassSpeicficBBox(const map<int, vector<NormalizedBBox> > all_gt_bbox,
                                   const map<int, int> selected_class, Blob<float>* bbox_data);
    template void GenerateClassSpeicficBBox(const map<int, vector<NormalizedBBox> > all_gt_bbox,
                                            const map<int, int> selected_class, Blob<double>* bbox_data);

    template <typename Dtype>
    void GenerateClassSpeicficBBox(const map<int, vector<NormalizedBBox> > all_gt_bbox,
                                   const map<int, int> selected_class, Blob<Dtype>* bbox_blob){
//        int num_cls_specific_box = 0;
        map<int, std::vector<caffe::NormalizedBBox>>::const_iterator iter;
//        for (iter = all_gt_bbox.begin(); iter != all_gt_bbox.end(); ++iter){
//            int img_id = iter->first;
//            vector<NormalizedBBox> bboxes_im = iter->second;
//            for (int i = 0; i < bboxes_im.size(); ++i){
//                int label = bboxes_im[i].label();
//                if (label == selected_class.at(img_id)) {
//                    num_cls_specific_box++;
//                }
//            }
//        }
...
//        bbox_blob->Reshape(bbox_data_shape);
        ...
        for (iter = all_gt_bbox.begin(); iter != all_gt_bbox.end(); ++iter){
            ...
            for (int i = 0; i < bboxes_im.size(); ++i){
                int label = bboxes_im[i].label();
                if (label == selected_class.at(img_id)) {
                    bbox_data[idx++] = img_id;
                    bbox_data[idx++] = label;
                    bbox_data[idx++] = instance_id++;
                    bbox_data[idx++] = bboxes_im[i].xmin();
                    bbox_data[idx++] = bboxes_im[i].ymin();
                    bbox_data[idx++] = bboxes_im[i].xmax();
                    bbox_data[idx++] = bboxes_im[i].ymax();
                    bbox_data[idx++] = bboxes_im[i].difficult();
                }
            }
        }
    }

    template void GenerateClassSpeicficBBox(const map<int, LabelBBox > selected_cls_bboxes,
                                            Blob<float>* bbox_blob);
    template void GenerateClassSpeicficBBox(const map<int, LabelBBox > selected_cls_bboxes,
                                            Blob<double>* bbox_blob);

    template <typename Dtype>
    void GenerateClassSpeicficBBox(const map<int, LabelBBox > selected_cls_bboxes,
                                   Blob<Dtype>* bbox_blob){
        map<int, LabelBBox>::const_iterator iter;

        Dtype* bbox_data = bbox_blob->mutable_cpu_data();

        int idx = 0;
        for (iter = selected_cls_bboxes.begin(); iter != selected_cls_bboxes.end(); ++iter){
            int img_id = iter->first;
            LabelBBox label_bbox = iter->second;
            LabelBBox::iterator it;
            int instance_id = 0;
            for (it = label_bbox.begin(); it != label_bbox.end(); ++it){
                int label = it->first;
                vector<NormalizedBBox> bboxes_im = it->second;
    ...
    int SelectClsBBoxes(const map<int, vector<NormalizedBBox> > all_gt_bboxes,
                         const map<int, int> selected_labels, map<int, LabelBBox>* selected_cls_bboxes){
        ...
        for (iter = all_gt_bboxes.begin(); iter != all_gt_bboxes.end(); ++iter){
            ...
            // Check if the img_id image has bboxes.
            if(selected_labels.find(img_id)==selected_labels.end()){
                continue;
            }
            ...
            for (int i = 0; i < bboxes_im.size(); ++i){
                int label = bboxes_im[i].label();
                if (label == selected_labels.at(img_id)) {
                    label_bbox[label].push_back(bboxes_im[i]);
                }
            }

            int num_bbox = (int) label_bbox[selected_labels.at(img_id)].size();
            if ( num_bbox <= 1){
                (*selected_cls_bboxes)[img_id] = label_bbox;
                num_all_bboxes += label_bbox[selected_labels.at(img_id)].size();
            } else {
                erase_num = rand() % num_bbox;
                if (erase_num == 0) {
                    (*selected_cls_bboxes)[img_id] = label_bbox;
                    num_all_bboxes += label_bbox[selected_labels.at(img_id)].size();
                } else {
                    auto seed = std::chrono::system_clock::now().time_since_epoch().count();
                    std::shuffle(label_bbox[selected_labels.at(img_id)].begin(), label_bbox[selected_labels.at(img_id)].end(),
                                 std::default_random_engine(seed));
                    vector<NormalizedBBox>::iterator it = label_bbox[selected_labels.at(img_id)].begin();
                    for (int k = 0; k < erase_num; k++){
                        label_bbox[selected_labels.at(img_id)].erase(it++);
                    }
                    (*selected_cls_bboxes)[img_id] = label_bbox;
                    num_all_bboxes += label_bbox[selected_labels.at(img_id)].size();
                }
        ...
        return num_all_bboxes;
    }

    void SelectClsBBoxes(BBoxSegDatum& bbox_seg_datum_ori, BBoxSegDatum* bbox_seg_datum){
//        std::cout << "start selectclsBBox" << std::endl;

        // Randomly select a class of annotation group
        bbox_seg_datum->clear_annotation_group();
        bbox_seg_datum->clear_seg_datum();

        int num_cls = bbox_seg_datum_ori.annotation_group().size();

//        std::cout << "num_cls" << num_cls << std::endl;

        srand((unsigned)time(0));
        int select_group_id = rand() % num_cls;



        AnnotationGroup* annotation_group = bbox_seg_datum->add_annotation_group();
        annotation_group->set_group_label(bbox_seg_datum_ori.annotation_group().Get(select_group_id).group_label());

//        std::cout << bbox_seg_datum->annotation_group_size() << std::endl;
//        std::cout << "select annotation group" << std::endl;

        // Randomly select a subset of instances
        int num_instance = bbox_seg_datum_ori.annotation_group().Get(select_group_id).annotation().size();
        CHECK(num_instance > 0) << "Instance number must be greater than 0.";
        // randomly generate random select number
        int num_select_instance;
        if (num_instance == 1){
            num_select_instance = 1;
        } else {
            num_select_instance = rand() % num_instance + 1;
        }
        // randomly generate a set of instance indexes
        vector<int> instance_ids;
        for (int i = 0; i < num_instance; ++i){
            instance_ids.push_back(i);
        }
        auto seed = std::chrono::system_clock::now().time_since_epoch().count();
        std::shuffle(instance_ids.begin(), instance_ids.end(), std::default_random_engine(seed));
        instance_ids.erase(instance_ids.begin()+num_select_instance, instance_ids.end());
        Annotation* annotation = NULL;
        for (int i = 0; i < num_select_instance; ++i){
            annotation = annotation_group->add_annotation();
            annotation->CopyFrom(bbox_seg_datum_ori.annotation_group().Get(select_group_id).annotation().Get(instance_ids[i]));
        }

//        std::cout << "select a subset of instances" << std::endl;
//        std::cout << bbox_seg_datum->annotation_group().Get(0).annotation_size() << std::endl;

        // copy the image data
        SegDatum* seg_datum = bbox_seg_datum->mutable_seg_datum();
        seg_datum->set_data(bbox_seg_datum_ori.seg_datum().data());
        seg_datum->set_channels(bbox_seg_datum_ori.seg_datum().channels());
        seg_datum->set_height(bbox_seg_datum_ori.seg_datum().height());
        seg_datum->set_width(bbox_seg_datum_ori.seg_datum().width());
        seg_datum->set_encoded(true);

//        std::cout << "copy the image data" << std::endl;

        // Merge all selected instance masks into a binary segmentation mask
        int im_height = seg_datum->height();
        int im_width = seg_datum->width();
        cv::Mat segmentation_mask(im_height, im_width, CV_8UC1, cv::Scalar::all(0));
        for (int i = 0; i < annotation_group->annotation_size(); ++i){
            const string& mask_i = annotation_group->annotation(i).mask();
            std::vector<char> vec_data(mask_i.c_str(), mask_i.c_str() + mask_i.size());
            int cv_read_flag = CV_LOAD_IMAGE_GRAYSCALE;
            cv::Mat cv_mask_i = cv::imdecode(vec_data, cv_read_flag);
            if (!cv_mask_i.data) {
                LOG(ERROR) << "Could not decode datum ";
            }
            CHECK_EQ(im_height, cv_mask_i.rows) << "image height and mask height mismatches.";
            CHECK_EQ(im_width, cv_mask_i.cols) << "image width and mask width mismatches.";
//            cv::addWeighted(segmentation_mask, 1, cv_mask_i, 1, 0.0, segmentation_mask);

            for (int h = 0; h < im_height; ++h) {
                for (int w = 0; w < im_width; ++w){
                    int label = int(segmentation_mask.at<uchar>(h,w));
                    int label_i = int(cv_mask_i.at<uchar>(h,w));
                    if (label + label_i > 0) {
                        segmentation_mask.at<uchar>(h,w) = uchar(int(1));
                    } else {
                        segmentation_mask.at<uchar>(h,w) = uchar(int(0));
                    }
                }
            }
        }

//        std::cout << "binary mask" << std::endl;

        // encode the segmentation mask cv mat into the seg_datum
        std::vector<uchar> buf;
        cv::imencode(".png", segmentation_mask, buf);
...
//        std::cout << "endcode" << std::endl;
    ...
    template void GenerateBinaryMask(const float* seg_data,  const map<int, int> selected_labels, float* binary_seg_data,
                            const int ignore_label_, const int num, const int height, const int width);
    template void GenerateBinaryMask(const double* seg_data,  const map<int, int> selected_labels, double* binary_seg_data,
                            const int ignore_label_, const int num, const int height, const int width);

    template <typename Dtype>
    void GenerateBinaryMask(const Dtype* seg_data,  const map<int, int> selected_labels, Dtype* binary_seg_data,
                            const int ignore_label_, const int num, const int height, const int width){
        for (int i = 0; i < num; ++i){

            // Check if the i image has bbox
            if(selected_labels.find(i) == selected_labels.end()){
//                std::cout << "image " << i << " does not have bboxes." << std::endl;
                continue;
            }

            int label_i = selected_labels.at(i);
            for (int j = 0; j < height*width; ++j){
                int label_seg_data = static_cast<int>(seg_data[i * width * height + j]);
                if (label_i == label_seg_data){
                    binary_seg_data[i * width * height + j] = Dtype(1);
                } else if (label_seg_data == ignore_label_){
                    binary_seg_data[i * width * height + j] = Dtype(ignore_label_);
                } else {
                    binary_seg_data[i * width * height + j] = Dtype(0);
                }
            }
//            std::cout << "image " << i << " done." << std::endl;
    ...
    template void GenerateBinaryMask(const float* seg_data,  const map<int, LabelBBox> selected_cls_bboxes, float* binary_seg_data,
                                     const int ignore_label_, const int num, const int height, const int width);
    template void GenerateBinaryMask(const double* seg_data,  const map<int, LabelBBox> selected_cls_bboxes, double* binary_seg_data,
                                     const int ignore_label_, const int num, const int height, const int width);

    template <typename Dtype>
    void GenerateBinaryMask(const Dtype* seg_data,  const map<int, LabelBBox> selected_cls_bboxes, Dtype* binary_seg_data,
                            const int ignore_label_, const int num, const int height, const int width){
        map<int, int> label_num;
        for (int i = 0; i < num; ++i){
            LabelBBox label_bbox = selected_cls_bboxes.at(i);
            LabelBBox::iterator it;
            for (it = label_bbox.begin(); it != label_bbox.end(); ++it){
                vector<NormalizedBBox> bboxes = it->second;
                for (int b = 0; b < bboxes.size(); ++b){
                    int xmin = (int) floor(bboxes[b].xmin() * width);
                    int ymin = (int) floor(bboxes[b].ymin() * height);
                    int xmax = (int) ceil(bboxes[b].xmax() * width);
                    int ymax = (int) ceil(bboxes[b].ymax() * height);
                    label_num.clear();
                    for (int h = ymin; h < ymax; h++){
                        for (int w = xmin; w < xmax; w++){
                            Dtype label = seg_data[i*width*height + h*width + w];
                            if (label != ignore_label_ && label > 0){
                                if (label_num.find(label) != label_num.end()){
                                    label_num[label] += 1;
                                } else {
                                    label_num[label] = 1;
                                }
                    ...
                    if (label_num.size() > 1){
                        ...
                        for (it_l=label_num.begin(); it_l != label_num.end(); it_l++){
                            if (it_l->second > max_num){
                                max_label = it_l->first;
                                max_num = it_l->second;
                            }
                        }
                    } else {
                        max_label = label_num.begin()->first;
                    }
                    ...
                    for (int h = ymin; h < ymax; h++){
                        for (int w = xmin; w < xmax; w++){
                            Dtype label = seg_data[i*width*height + h*width + w];
                            if (label == max_label){
                                binary_seg_data[i*width*height + h*width + w] = Dtype(1);
                            }
//                            else if(label == ignore_label_){
//                                binary_seg_data[i*width*height + h*width + w] = label;
//                            }
                            else {
                                binary_seg_data[i*width*height + h*width + w] = Dtype(0);
                            }
                        }
                    }
    ...
    template void GenerateClass(const map<int, int> selected_labels, const int num_class, float* cls_data);
    template void GenerateClass(const map<int, int> selected_labels, const int num_class, double* cls_data);

    template <typename Dtype>
    void GenerateClass(const map<int, int> selected_labels, const int num_class, Dtype* cls_data) {
//        for (int i = 0; i < selected_labels.size(); ++i){
//            int label = selected_labels.at(i) - 1;
//            CHECK_GE(label, 0) << "Class label can not be negative.";
//            cls_data[i * num_class + label] = Dtype(1);
//        }

        map<int, int>::const_iterator it;
        for (it = selected_labels.begin(); it != selected_labels.end(); ++it){
            int img_id = it->first;
            int label = it->second - 1;
            CHECK_GE(label, 0) << "Class label can not be negative.";
            cls_data[img_id * num_class + label] = Dtype(1);
        }
    }

    bool InBBox(const int x, const int y, const int xmin, const int ymin, const int xmax, const int ymax) {
