...
cv::Mat ApplyResize(const cv::Mat& in_img, const ResizeParameter& param) {
  ...
  return  out_img;
    ...
    void unique_mask_labels(const cv::Mat mask, vector<int> *array_int){
      // list all different mask labels
      // put the mat values into a vector
      std::vector<uchar> array;
      if (mask.isContinuous()) {
        array.assign(mask.datastart, mask.dataend);
      } else {
        for (int i = 0; i < mask.rows; ++i) {
          array.insert(array.end(), mask.ptr<uchar>(i), mask.ptr<uchar>(i)+mask.cols);
        }
      }
      // conver unchar to int type
//    vector<int> array_int;
      (*array_int).clear();
      for (auto e : array) {
        (*array_int).push_back(static_cast<int>(e));
      }
      // erase the duplicate values
      sort((*array_int).begin(), (*array_int).end());
      (*array_int).erase(unique((*array_int).begin(), (*array_int).end()), (*array_int).end());
    }

    int min_label(int label, vector<int> mask_labels){
      int min_label = -1;
      int distance = 1000;
      for (int i = 0; i < mask_labels.size(); ++i){
        if (abs(mask_labels[i] - label) < distance){
          distance = abs(mask_labels[i] - label);
          min_label = mask_labels[i];
        }
    ...
    void change_mask_labels(cv::Mat *mask, vector<int> mask_labels){
      int rows = (*mask).rows;
      int cols = (*mask).cols;
      for (int i = 0; i < rows; ++i) {
        for (int j = 0; j < cols; ++j){
          int label = int((*mask).at<uchar>(i,j));
          (*mask).at<uchar>(i,j) = uchar(min_label(label, mask_labels));
        }
      }
    }
